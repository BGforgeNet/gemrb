			   ...
			   unsigned short* secondary)
{
	...
	Animation ani = Animation(std::move(frames), ANI_DEFAULT_FRAMERATE * core->GetAnimationSpeedScale());
	...
	if (secondary) {
		...
		Animation sec = Animation(std::move(frames), ANI_DEFAULT_FRAMERATE * core->GetAnimationSpeedScale());
